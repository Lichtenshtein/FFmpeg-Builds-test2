...
ffbuild_dockerbuild() {
    ...
    if [[ -f "$PC_FILE" ]]; then
        ...
        sed -i 's/^Libs:.*/& -lpangocairo-1.0 -lpango-1.0 -lcairo -lcairo-gobject -lglib-2.0 -lgobject-2.0 -lxml2 -lintl -lcharset -liconv -luserenv -lusp10 -lsetupapi -lruntimeobject -lntdll -lmsimg32 -lgdi32 -lstdc++/' "$PC_FILE"
    fi
