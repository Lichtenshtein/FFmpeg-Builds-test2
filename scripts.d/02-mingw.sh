...
ffbuild_dockerbuild() {
    ...
    local CRT_CFLAGS="-O3 -march=${CPU_ARCH} -mtune=${CPU_TUNE} -mavx2 -mfma -pipe -g -fno-lto"
    ...
    cd mingw-w64-libraries/winpthreads
        if [[ "$USE_LTO" == "1" ]]; then
            ...
            local PTHREAD_CFLAGS="-O3 -march=${CPU_ARCH} -mtune=${CPU_TUNE} -mavx2 -mfma -pipe -g ${USELTO}${USELTO_C}"
        else
            export AR="${FFBUILD_TOOLCHAIN}-ar"
            export RANLIB="${FFBUILD_TOOLCHAIN}-ranlib"
