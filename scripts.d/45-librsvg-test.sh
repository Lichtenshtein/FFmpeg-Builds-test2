...
ffbuild_dockerbuild() {
    ...
    if [[ -f "$PC_FILE" ]]; then
        ...
        sed -i 's/^Libs:.*/& -lxml2 -lpangocairo-1.0 -lpango-1.0 -lcairo -lgobject-2.0 -lglib-2.0 -lintl -liconv -lws2_32 -luserenv -lusp10 -lshlwapi -lsetupapi -lruntimeobject -lmsimg32 -lgdi32 -luser32 -lbcrypt/' "$PC_FILE"
    fi
}
