...
ffbuild_dockerbuild() {
    ...
    # —оздаем эталонный shaderc.pc, который реально будет работать при статической линковке FFmpeg
    mkdir -p "$PC_DIR"
...
EOF
    ...
    # ƒублируем его в shaderc_combined.pc и shaderc_static.pc дл€ совместимости
    ...
    (
        ...
        # —обираем цель glslc_exe Ч в последних верси€х бинарник прив€зан к ней
        ninja $NINJA_V glslc glslc_exe || true

        # ищем любой исполн€емый файл с именем glslc в текущей директории
