...
for STAGE in "${active_scripts[@]}"; do
    ...
    to_df "    --mount=type=cache,id=downloads-${TARGET},target=${CONTAINER_ROOT}/.cache/downloads \\"
...
if [[ "${SKIP_FFMPEG}" == "1" ]]; then
    ...
    to_df "RUN --mount=type=cache,id=ccache-${TARGET},target=${CCACHE_DIR} \\"
    to_df "    --mount=type=cache,id=ffmpeg-src-${TARGET},target=/builder/ffbuild/ffmpeg \\"
