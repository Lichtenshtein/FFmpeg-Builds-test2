...
CONF_FLAGS=(
    ...
    --extra-cflags="-O2 -march=broadwell -mtune=generic -pipe -mms-bitfields -fstack-protector-strong -mstackrealign -std=gnu23 -I/opt/ffbuild/include -D_FORTIFY_SOURCE=2"
    --extra-cxxflags="-O2 -march=broadwell -mtune=generic -pipe -mms-bitfields -fstack-protector-strong -mstackrealign -std=gnu++20 -I/opt/ffbuild/include -D_FORTIFY_SOURCE=2"
    # --extra-ldflags="${ASAN_LDFLAGS}${FINAL_LDFLAGS} -march=${CPU_ARCH} -mtune=${CPU_TUNE} -mavx2 -mfma"
    --extra-ldflags="${ASAN_LDFLAGS}${FINAL_LDFLAGS} -march=${CPU_ARCH}"
